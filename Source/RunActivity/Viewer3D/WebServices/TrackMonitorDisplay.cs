    ...
    public static class TrackMonitorDisplay
    {
        ...
        private static void DrawMPModeInfo(List<ListLabel> labels, Train.TrainInfo trainInfo, bool useMetric)
        {
            ...
            if (trainInfo.direction != 1)
            {
                ...
                float markerIntervalM = DrawDistanceMarkers(labels, distanceFactor, zeroObjectPointTop, useMetric, numberOfMarkers: 4, direction: TrainDirection.Forward);
            ...
            if (trainInfo.direction !=0)
            {
                ...
                // Draw backward items
                DrawTrackItems(labels, trainInfo.ObjectInfoBackward, zeroObjectPointBottom, distanceFactor, markerIntervalM, useMetric, direction: TrainDirection.Backward);
            }
        ...
        private static void DrawManualModeInfo(List<ListLabel> labels, Train.TrainInfo trainInfo, bool useMetric)
        {
            ...
            // Draw fixed distance indications
            float markerIntervalM = DrawDistanceMarkers(labels, distanceFactor, zeroObjectPointTop, useMetric, numberOfMarkers: 3, direction: TrainDirection.Forward);
            DrawDistanceMarkers(labels, distanceFactor, zeroObjectPointBottom, useMetric, numberOfMarkers: 3, direction: TrainDirection.Backward); // no return required
        ...
        private static float DrawDistanceMarkers(List<ListLabel> labels, float distanceFactor, int zeroPoint, bool useMetric, int numberOfMarkers, TrainDirection direction)
        {
        ...
        private static void DrawTrackItems(List<ListLabel> labels, IEnumerable<Train.TrainObjectItem> itemList, int zeroPoint, float distanceFactor, float markerIntervalM, bool useMetric, TrainDirection direction)
        {
