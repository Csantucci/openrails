    ...
    public class SuperElevationManager
    {
        ...
        public static bool DecomposeStaticSuperElevation(Viewer viewer, TrackObj trackObj, WorldPosition worldMatrixInput, List<DynamicTrackViewer> dTrackList, string shapeFilePath)
        {
            ...
            try
            {
                ...
                dontRender = shape.TunnelShape && !viewer.Simulator.TRK.Tr_RouteFile.ChangeTrackGauge;
            ...
            int trpIndex = DynamicTrackViewer.GetBestTrackProfile(viewer, shapeFilePath, shape == null ? false : shape.TunnelShape);
...
/*                    if (section.SectionCurve == null && !viewer.Simulator.TRK.Tr_RouteFile.ChangeTrackGauge)
                    ...
                    tmp = FindSuperElevationSection(viewer, section, trackObj.UID, root, nextRoot, out bool reversed);
    ...
    public class SuperElevationViewer : DynamicTrackViewer
    {
        public SuperElevationViewer(Viewer viewer, WorldPosition position, WorldPosition endPosition, float radius, float angle,
            TrProfile trProfile = null, Interpolator elevs = null, float rollOffset = 0, bool reversed = false, bool inTunnel = false)
            : base(viewer, position)
        {
            // Instantiate classes
            Primitive = new SuperElevationPrimitive(viewer, position, endPosition, radius, angle, trProfile, elevs, rollOffset, reversed, inTunnel);
        }
    }
    ...
    public class SuperElevationPrimitive : DynamicTrackPrimitive
    {
        ...
        public SuperElevationPrimitive(Viewer viewer, WorldPosition worldPosition, WorldPosition endPosition, float radius,
            float angle, TrProfile trProfile = null, Interpolator elevs = null, float rollOffset = 0, bool reversed = false, bool inTunnel = false)
            : base()
        {
            Reversed = reversed;
