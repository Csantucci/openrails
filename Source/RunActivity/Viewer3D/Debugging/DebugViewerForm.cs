...
namespace Orts.Viewer3D.Debugging
{
    ...
    public partial class DispatchViewer : Form
   {
      ...
      public readonly Simulator simulator;
        ...
        // Color theme and selection: Dark | white
        public ExTheme Extheme;
        public bool ExThemeSelect = false;
	  ...
	  private System.Drawing.Point LastCursorPosition = new System.Drawing.Point();
		...
		public Pen grayPen = new Pen(Color.Gray);
                             public Pen PlatformPen = new Pen(Color.Cyan);
	...
	public Train PickedTrain;
	  ...
	  public Viewer Viewer;
        ...
        public DispatchViewer(Simulator simulator, Viewer viewer)
        {
            ...
            Extheme = new ExTheme(ExThemeSelect);
        }
      ...
      void DetermineLocations()
      {
          ...
          if (oldHeight != this.Height || oldWidth != AnchorPoint.Left)//use the label "Res" as anchor point to determine the picture size
          {
              oldWidth = AnchorPoint.Left; oldHeight = this.Height;
              IM_Width = AnchorPoint.Left - 20;
         ...
         using(Graphics g = Graphics.FromImage(pbCanvas.Image))
         {
            ...
            if (true/*showPlayerTrain.Checked*/)
            {
				...
				foreach (Train t in selectedTrainList)
				{
					...
					if (drawRed > redTrain) pathPen.Color = Color.Aqua; //more than the red should be drawn, thus draw in blue
	  ...
	  private void btnSeeInGameClick(object sender, EventArgs e)
	  {
		  if (PickedTrain != null) ClickedTrain = true;
                ...
                private void uibThemeChance_Click(object sender, EventArgs e)
                {
                      if (ExThemeSelect)
                      {
                             Extheme.setThemeColoe(false);
                             ExThemeSelect = false;
                             TimetableWindow.SetControls();
                      }
                      else
                      {
                             Extheme.setThemeColoe(true);
                             ExThemeSelect = true;
                             TimetableWindow.SetControls();
                      }    
