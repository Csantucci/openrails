    ...
    public partial class SoundDebugForm : Form
    {
        ...
        void UpdateContent()
        {
            ...
            foreach (var src in soundSources.Values)
            {
                ...
                {
                    SoundSource ss = null;
                    SoundSource ss1 = null;
                    if (ssb is SoundSource)
                    {
                        ss = (SoundSource)ssb;
                        tssflag = 0;
                    }
                    else if (ssb is TrackSoundSource)
                    {
                        var ts = (TrackSoundSource)ssb;
                        if (ts._activeInSource != null)
                        {
                            ss = ts._activeInSource;
                        }
                        if (ts._activeOutSource != null)
                        {
                            ss1 = ts._activeOutSource;
                            tssflag = 1;
                        }
                    }
                    else continue;
                    // If track sound source loop twice
                    // loop the 


                    for (int i = 0; i <= tssflag; i++)
                    {
                        if (tssflag == 1)
                        {
                            ss = ss1;
