    ...
    public class SuperElevation
    {
        ...
        public SuperElevation(Simulator simulator)
        {
            ...
            foreach (var node in simulator.TDB.TrackDB.TrackNodes)
            {
                ...
                int j = 0; int count = node.TrVectorNode.TrVectorSections.Length;
                foreach (TrVectorSection vectorSec in node.TrVectorNode.TrVectorSections)
                {
                    ...
                    {
                        TrackShape sha;
                        if (simulator.TSectionDat.TrackShapes.TryGetValue(vectorSec.ShapeIndex, out sha) && sha.MainRoute != int.MaxValue)
                            continue;
                        if ((j == 1 && simulator.TDB.TrackDB.TrackNodes[node.TrPins[0].Link].TrEndNode == true) ||
                            (j == count && simulator.TDB.TrackDB.TrackNodes[node.TrPins[1].Link].TrEndNode == true))
                            continue;
                    }
                    SectionCurve theCurve = sec.SectionCurve;
                    if (sec.SectionSize != null && Math.Abs(sec.SectionSize.Width - simulator.RouteTrackGaugeM) > 0.2
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (i < count - 1)
                    {
                        ...
                        if (nextSec != null)
                        {
                            ...
                            nextStraight = nextSec.SectionSize.Length != 0 ? nextSec.SectionSize : null;
                        }
                    }
